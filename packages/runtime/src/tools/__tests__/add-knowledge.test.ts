...
import { createAddKnowledgeTool } from "@diligent/runtime/tools";
...
describe("add_knowledge tool", () => {
  ...
  it("does not request approval", async () => {
    ...
    const result = await tool.execute({ type: "pattern", content: "No approval required", confidence: 0.9 }, makeCtx());

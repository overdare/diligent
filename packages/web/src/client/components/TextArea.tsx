...
export function TextArea({ maxRows = 6, className, onChange, ...props }: TextAreaProps) {
  ...
  return (
    <textarea
      ...
      rows={1}
      className={cn(
        "flex-1 resize-none rounded-md border border-text/20 bg-bg px-3 py-2 text-sm text-text placeholder:text-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent",
        className,
      )}
      onChange={(e) => {
        onChange?.(e);

  ...
  return (
    <div className="overflow-hidden rounded-lg border border-text/10 bg-bg/60 font-mono text-xs">
      ...
      <div className="flex items-center gap-2 border-b border-text/10 bg-surface/60 px-3 py-2">
        ...
        {pattern ? (
          <span className="rounded bg-accent/10 px-1.5 py-0.5 text-accent/90">/{pattern}/</span>
        ) : null}
        {metaParts.length > 0 ? (
          <span className="min-w-0 truncate text-muted/70">{metaParts.join(" in ")}</span>
        ) : null}
...
function MatchLine({ line, pattern, isError }: { line: string; pattern?: string; isError: boolean }) {
  ...
  if (!match) {
    return (
      <div className={`truncate leading-relaxed ${isError ? "text-danger/80" : "text-text/60"}`}>{line}</div>
    );
  }

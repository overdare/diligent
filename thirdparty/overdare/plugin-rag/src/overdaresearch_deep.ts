...
export const parameters = z.object({
  action: z
    .enum(["origin-file"])
    .describe("origin-file = fetch full content of original files by GCS URL."),
  urls: z
    .array(z.string())
    .min(1)
    .max(10)
    .describe(
      "For origin-file: 1-10 GCS file URLs (lua-script-bucket or ovdr-docs-bucket).",
    ),
})
...
interface ToolContext {
  toolCallId: string
  signal: AbortSignal
  approve: (req: { permission: "read" | "write" | "execute"; toolName: string; description: string; details?: Record<string, unknown> }) => Promise<"once" | "always" | "reject">
}
...
async function originFile(urls: string[], signal: AbortSignal, token: string): Promise<ToolResult> {
  for (const url of urls) {
    if (!url.startsWith(LUA_BUCKET) && !url.startsWith(DOCS_BUCKET)) {
      throw new Error(
        `URL not from allowed bucket: ${url}\n` +
  ...
  const data = (await fetcher(
    `${BASE_URL}/api/chat/rag/origin-file?${params}`,
    signal,
    token,
  )) as OriginFileResponse

...
| `gemini` | Use Gemini's configured API key and image model. |
...
only their selected provider; neither falls back to the other.
...
`source` (`chatgpt-oauth` or `gemini-api`), and an image preview. For ChatGPT, `requestedModel`
and `requestedBackground` record the pinned request. `model` and `background` are included only
when reported by the backend; they are not inferred from the prompt, filename, or HTTP success.

...
Gemini also exposes no count input and returns one entry in the same `images` array contract.
...
`source` (`chatgpt-oauth` or `gemini-api`), and image previews. ChatGPT `requestedModel` and
`requestedBackground` record the request. `model` and `background` are included only when reported by the backend;

...
The `geometry-recipe` skill and agent are restored from the original definitions
at `4dac65bc`, preserving their native geometry rules, material/UV/resource advice,
inspection guidance, and input/output structure. Only the retired recipe RPC flow,
its synchronous report assumptions, and file-to-Source synchronization are adapted

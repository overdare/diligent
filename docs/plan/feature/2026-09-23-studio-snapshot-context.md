...
Delivery verification on `feat/ovdr-15298-snapshot-context`, based on `origin/main` at `4368a4c5`: all 342 affected tests passed across 14 Runtime, E2E, and Studio files. The working patch was preserved exactly when moving from the earlier model-feature branch to the current main baseline.

Final review found that a failed metadata write could expose a manual or safety map as a legacy automatic snapshot. Capture now prepares temporary files, publishes metadata first, and exposes the map last. Failed publication cleans temporary files and new metadata; existing IDs are protected, including metadata-only IDs left by interruption. Background summary metadata uses atomic replacement. Failure regressions cover both publication boundaries, preserved default rollback targets, existing IDs, reserved indexes, and intact metadata after summary replacement failure.

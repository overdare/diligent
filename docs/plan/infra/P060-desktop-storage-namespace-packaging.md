...
The runtime changes in this plan should stay limited to reusable namespace-aware path abstractions, and P060 activates them only through the packaged desktop build flow. In other words, the abstraction may be reusable by future callers, while the required behavior change in this plan remains scoped to packaged desktop / packaged sidecar execution.

When a packaged product first switches to a non-default namespace, the desktop shell / Tauri layer should perform a one-time conditional migration only if the target namespace does not exist and the legacy `.diligent` namespace does exist. Runtime stays migration-free in this plan.

## Prerequisites

- Packaging pipeline MVP (P044)
- Desktop runtime auto-update architecture and path ownership in `apps/overdare-agent/src-tauri/src/update.rs`, `init.rs`, and `sidecar.rs`
- Existing branded packaging metadata resolution in `apps/overdare-agent/scripts/lib/project-name.ts`

## Artifact

```text
$ bun run apps/overdare-agent/scripts/package.ts --version 0.3.0
...
The same packaged binary does not read from or write to ~/.diligent/ or <startup-cwd>/.diligent/ after desktop-owned migration/selection settles on the branded namespace.
```
...
Desktop/Tauri-owned conditional first-run migration rule:
...
| Area | What Changes |
|------|-------------|
| `apps/overdare-agent/scripts/lib/project-name.ts` | Extend app-owned desktop metadata resolution to include a packaged desktop storage namespace. |
| `apps/overdare-agent/scripts/package.ts` | Pass namespace env into sidecar build and Tauri build, and include it in build fingerprints. |
| `apps/overdare-agent/scripts/build-exe-only.ts` | Pass the same namespace env through the exe-only packaging flow. |
...
| `packages/web/src/shared/image-routes.ts` and `packages/web/src/server/index.ts` | Keep packaged sidecar persisted image and route behavior aligned with the selected local namespace rooted at startup cwd, assuming desktop already handled migration before launch. |
| `apps/overdare-agent/src-tauri/src/init.rs` | Replace hardcoded `~/.diligent` setup/deploy paths with a single namespace-aware global directory helper and desktop-owned migration ordering. |
| `apps/overdare-agent/src-tauri/src/sidecar.rs` | Replace hardcoded desktop log/update/runtime paths with the shared namespace-aware helper and perform local namespace migration before launching the sidecar. |
| `apps/overdare-agent/src-tauri/src/update.rs` | Replace hardcoded desktop config/update paths with the shared namespace-aware helper. |
| `apps/overdare-agent/src-tauri/src/lib.rs` | Use the same namespace-aware helper for startup/init log and setup surfaces, including migration coordination. |
| `apps/overdare-agent/test/scripts/project-name.test.ts` | Add metadata-resolution tests for packaged desktop storage namespace. |
| `apps/overdare-agent/test/scripts/version.test.ts` or new packaging-script tests | Verify package scripts propagate namespace env for desktop packaging flows. |
...
### apps/overdare-agent/scripts/lib/

| File | Action | Description |
|------|--------|------------|
| `project-name.ts` | MODIFY | Resolve optional storage namespace metadata from app package config and expose a normalized helper. |

### apps/overdare-agent/scripts/
...
| File | Action | Description |
|------|--------|------------|
| `server/index.ts` | MODIFY | Consume desktop-selected namespace-aware local paths consistently from desktop-passed startup cwd without owning migration. |
...
### apps/overdare-agent/src-tauri/src/
...
### apps/overdare-agent/test/scripts/
...
**Files:** `apps/overdare-agent/scripts/lib/project-name.ts`, `apps/overdare-agent/test/scripts/project-name.test.ts`
**Decisions:** D033, D042
...
export function resolveDesktopStorageNamespace(appDir?: string): string {
  const configured = readPackageJson(join(appDir ?? APP_ROOT, "package.json"))?.diligent?.desktopStorageNamespace;
...
**Verify:** `bun test apps/overdare-agent/test/scripts/project-name.test.ts`

### Task 2: Thread namespace through desktop package scripts

**Files:** `apps/overdare-agent/scripts/package.ts`, `apps/overdare-agent/scripts/build-exe-only.ts`
**Decisions:** D042
...
```typescript
const storageNamespace = resolveDesktopStorageNamespace();
...
**Verify:** targeted packaging-script tests for resolved env values; manual dry run with and without ``.

### Task 3: Add desktop-owned conditional namespace migration

**Files:** `apps/overdare-agent/src-tauri/src/lib.rs`, `apps/overdare-agent/src-tauri/src/init.rs`, `apps/overdare-agent/src-tauri/src/sidecar.rs`
**Decisions:** D042
...
This should be implemented as an explicit Rust helper inside the desktop/Tauri layer and used for both home-global state and startup-cwd-local state before the sidecar is launched. Runtime should only consume the selected namespace after desktop startup has already settled migration.
...
- local migration failure should fail packaged sidecar launch from the desktop shell rather than falling back to legacy or creating a fresh target namespace
...
**Verify:** desktop Rust unit tests for all three outcomes; manual migration smoke test from a machine/repo that only has `.diligent`.
...
- `packages/web/src/server/index.ts`: continue using `ensureDiligentDir(cwd)` / `loadRuntimeConfig(cwd, paths)`, but now those helpers must create and consume `./.overdare` when the packaged env is set, assuming desktop already handled any legacy migration before launch

Because the packaged sidecar is compiled with `DILIGENT_STORAGE_NAMESPACE`, these helper changes affect packaged desktop behavior while preserving existing `.diligent` bootstrap for ordinary dev/runtime flows. The helper itself is reusable, but P060 activates it only through the packaged desktop build/startup path. The required packaged result is symmetry: if global is `~/.overdare`, local must also be `./.overdare` relative to startup cwd. If only legacy `.diligent` data exists, desktop-owned Task 3 moves it first.

**Verify:** `bun test` for the focused runtime tests above, plus a packaged sidecar smoke test confirming it consumes `./.overdare` after desktop-owned migration when the packaged namespace is set.

### Task 5: Centralize desktop global-dir resolution in Rust

**Files:** `apps/overdare-agent/src-tauri/src/lib.rs`, `apps/overdare-agent/src-tauri/src/init.rs`, `apps/overdare-agent/src-tauri/src/sidecar.rs`, `apps/overdare-agent/src-tauri/src/update.rs`
**Decisions:** D042
...
Do not add dual-read behavior. After desktop-owned conditional migration, the packaged build should use exactly one namespace.

**Verify:** `cargo test` or `cargo check` in `apps/overdare-agent/src-tauri/`, plus focused unit tests for helper output and migration ordering.

### Task 6: Keep sidecar/runtime behavior aligned with the selected namespace

**Files:** `apps/overdare-agent/scripts/package.ts`, `apps/overdare-agent/src-tauri/src/sidecar.rs`, `apps/overdare-agent/src-tauri/src/update.rs`
**Decisions:** D042
...
This task assumes Task 3 and Task 4 are implemented. The preferred outcome is coherence: packaged desktop should not split its state between `~/.overdare` and `~/.diligent`, or between `./.overdare` and `./.diligent`. By the time the sidecar starts, any required migration should already be completed by the desktop shell.

**Verify:** manual packaged smoke test — install/run packaged desktop, confirm logs, updates, deployed bootstrap, sessions, knowledge, skills, and project config land under the selected namespace only, with one-time migration when only legacy data exists.
...
- the desktop shell/Tauri layer migrates legacy `.diligent` only when the branded target does not yet exist
...
3. If `<startup-cwd>/.overdare` is absent and `<startup-cwd>/.diligent` exists, the desktop shell moves `<startup-cwd>/.diligent` to `<startup-cwd>/.overdare` before launching the packaged sidecar.
...
11. If local migration fails, the desktop shell aborts packaged sidecar launch rather than falling back to legacy paths or creating a fresh target namespace.
...
|----------|-------------|-----|
| Unit | Namespace metadata parsing, normalization, and invalid-value failure | `bun test apps/overdare-agent/test/scripts/project-name.test.ts` |
| Unit | Packaging env propagation | `bun test` for updated/new script tests around package/build-exe-only helpers |
| Unit | Desktop-owned conditional migration helper | `cargo test` for focused Tauri migration helper coverage |
...
| Manual | Failure surfacing | confirm global migration failure opens the desktop window with an error, while local migration failure aborts sidecar launch from desktop |
...
| Target namespace directory is created before migration check | Migration never triggers and legacy data is orphaned | Run desktop-owned migration before any mkdir/create_dir_all/ensure path calls for the target namespace |
...
| Migration failure falls through to partial startup | Product appears launched but storage is split or blocked in non-obvious ways | Fail hard on invalid namespace values, surface global migration failure in-window, and fail local migration in the desktop shell before sidecar launch |

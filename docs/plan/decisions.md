...
## Incremental Architecture Decisions (Post D099)

### D100: MCP tool loading — `auto` mode with configurable lazy threshold
- **Decision**: MCP tools are exposed in two modes: **eager** (one tool per MCP tool, the classic pattern) and **lazy** (two proxy tools: `mcp_search_tools` and `mcp_run_tool`). The default setting is `auto`, which selects eager when the total catalog size is ≤ 20 tools and lazy when it exceeds the threshold. The threshold is configurable via `mcp.lazyThreshold` in `config.jsonc`.
- **Rationale**: Providing every MCP tool as an individual function definition is fine for small server sets, but with many servers the combined schema payload grows linearly and crowds the context window. The proxy pair (`mcp_search_tools` + `mcp_run_tool`) keeps the per-turn tool list constant regardless of how many MCP tools are installed — the agent searches by name/description and then calls through the run proxy. Splitting search and run into two separate tools (rather than one `tool_search_and_call` action-discriminated tool) matches the codex-rs `tool_search` design: the agent can refine search results before committing to a call. The `auto` default means small setups are unaffected and get native per-tool definitions, while large setups automatically switch to the proxy surface.
- **Alternatives considered**: Always-lazy (simple but regresses small setups that benefit from direct schema validation); always-eager (no context scaling); single combined search+call tool (conflates discovery with execution, harder to audit); dynamic mid-conversation tool registration (deferred — requires mutable per-turn tool set, currently not supported).
- **Consequence**: The tool-settings/catalog UI in TUI/Web always uses eager mode so per-tool toggles remain visible; only the runtime agent (`createRuntimeAgent`) respects `auto`/`lazy`.
- **References**: `packages/runtime/src/tools/mcp/types.ts` (`DEFAULT_MCP_LAZY_THRESHOLD`), `packages/runtime/src/tools/mcp/provider.ts` (`resolveLoading`), `packages/runtime/src/tools/mcp/tool-search.ts`, `packages/runtime/src/config/schema.ts` (`McpGlobalConfigSchema`)
- **Date**: 2026-07-01

### D101: Mode filtering strategy — disallowed-list over allow-list
- **Decision**: Tool availability in `plan` and `execute` modes is controlled by per-mode **disallowed sets** (`PLAN_MODE_DISALLOWED_TOOLS`, `EXECUTE_MODE_DISALLOWED_TOOLS`) rather than an explicit allowed set. The disallowed sets are derived at startup from tool capability metadata (`planModeDisallowed` / `executeModeDisallowed` flags on `TOOL_CAPABILITIES` entries). External tools (MCP, plugins) are allowed by default in all modes unless explicitly flagged.
- **Rationale**: An allow-list requires every new tool to explicitly opt in to each mode, which creates friction when adding external tools and risks silently excluding useful new capabilities. A disallowed-list is the safer default: new tools are available in all modes and only the tools with known mutation or unsafe semantics are excluded from restricted modes. This matches the principle of least surprise for MCP tool authors. The capability metadata approach (flags on `TOOL_CAPABILITIES`) collocates the mode policy with the tool definition, making it easy to audit and maintain.
- **Alternatives considered**: Allow-list per mode (default-deny, forces every tool to opt in — too restrictive for external tools); runtime mode-checking inside each tool handler (scattered, hard to audit); separate mode config file (out of band, drift risk).
- **Consequence**: When a new built-in tool is added that should be restricted in plan mode, the author must add `planModeDisallowed: true` to its `TOOL_CAPABILITIES` entry. Omitting the flag means the tool is available in all modes — which is the correct default for read-only tools.
- **References**: `packages/runtime/src/tools/tool-metadata.ts` (`PLAN_MODE_DISALLOWED_TOOLS`, `EXECUTE_MODE_DISALLOWED_TOOLS`, `TOOL_CAPABILITIES`), `packages/runtime/src/agent/mode.ts`
- **Date**: 2026-07-01

### D102: Durable server requests — user-input prompts survive client disconnect
- **Decision**: `PendingServerRequest` carries a `durable: boolean` flag. Durable requests (currently: user-input / `request_user_input` calls) are created with no timeout (`timeoutId: null`) and are re-delivered to any client that subscribes after the request was sent. The request stores its `params` and `threadId` so the app-server can replay the prompt on reconnect. Non-durable requests (e.g., approval prompts) time out normally.
- **Rationale**: A user-input request blocks the agent turn until the user responds. If the web client is closed or reloads mid-turn, a non-durable request would be silently dropped, leaving the agent stuck waiting for a response it will never receive. Durability ensures that a reconnecting client receives the pending prompt and can complete the interaction. This design allows the server to be the authoritative source of pending requests without needing a separate storage system — the in-memory `pendingServerRequests` map serves as the transient queue.
- **Alternatives considered**: Persist requests to session JSONL and replay on reconnect (heavier, adds I/O); time out all requests and let the agent retry (wastes context, may not recover cleanly); client-side re-send on reconnect (requires client to know which requests are in-flight).
- **Consequence**: The app-server must not prematurely clean up durable requests. Any client that subscribes to a thread with a pending durable request will receive the prompt immediately. When a response arrives, all clients for that thread are notified.
- **References**: `packages/runtime/src/app-server/server-requests.ts` (`PendingServerRequest.durable`, `PendingServerRequest.params`, `PendingServerRequest.threadId`)
- **Date**: 2026-07-01

### D103: Empty `allowed_tools` → inherit-all for sub-agent tool filtering
- **Decision**: When `spawn_agent` is called with `allowed_tools: []` (an empty array), this is normalized to `undefined`, which is semantically "inherit all parent tools". Only a non-empty `allowed_tools` array narrows the child's tool set. The normalization is applied by `normalizeAllowedTools()` in `packages/runtime/src/collab/spawn-agent.ts`.
- **Rationale**: An empty list as a filter would produce a tool-less agent, which is almost never the caller's intent. The natural English reading of omitting `allowed_tools` is "give the child agent the same tools as me", and an empty list is indistinguishable from an accidental omission. Normalizing to undefined provides the least-surprise default. Callers who genuinely want a tool-less agent (an edge case with no current use) can omit the tool entirely from the child's config through other mechanisms.
- **Alternatives considered**: Treat empty array as "no tools" (literal semantics — counterintuitive, likely to cause confusing zero-tool agents); require callers to explicitly omit the field (forces a schema change); validate and reject empty arrays (adds error surface without benefit).
...
### D105: Model identity is the provider-scoped pair `(provider, modelId)`
- **Decision**: A model is identified everywhere by the value object `ModelRef = { provider, modelId }`, not by a globally-unique model ID string. The catalog is provider-scoped (`provider -> modelId -> model card`) and permits the same `modelId` under different providers (e.g. `openai/gpt-5.5` and `chatgpt/gpt-5.5` are distinct cards). `ModelRefSchema` lives in `@diligent/protocol` and is used directly in every model-selection field (initialize/thread/turn/config params and responses, `AssistantMessage.model`); `ModelInfo.provider` uses `ProviderNameSchema`. Core `resolveModel(ref)` is strict — it requires provider context, throws a typed unknown-model error, and never infers capabilities from a bare ID. `Agent`/`RuntimeAgent` accept a resolved `Model`. ChatGPT sends canonical `gpt-*` IDs (the synthetic `chatgpt-*` prefix and `resolveChatGPTModelId()` are removed). Config persists `{ provider, modelId }` and merges it atomically. The canonical textual selector is `<provider>/<model-id>`; an unqualified ID resolves only when unique. Reference equality goes through `sameModelRef()`; never compare `modelId` alone or with `===`.
- **Rationale**: The old flat `MODEL_CARDS` array made a bare model ID do three unrelated jobs (catalog key, provider/auth routing key, upstream identifier), which forced synthetic IDs like `chatgpt-5.5` to preserve global uniqueness and produced an implicit `unknown -> anthropic` capability fallback. Provider-scoped identity lets the same provider-native model ID exist under multiple providers (OpenAI API vs. ChatGPT subscription, and a future Bedrock-hosted Claude) without renaming, removes prefix inference from steady state, and makes provider switches on the same model ID observable.
- **Consequence**: Prefix inference is confined to a single runtime-owned legacy migration adapter (`packages/runtime/src/model/legacy-model-ref.ts`) used only at persisted-config/session read boundaries — it maps known pre-P082 strings deterministically and never fabricates a card. Protocol breaks in place while `protocolVersion` stays `1` (per D096); there is no old-wire compatibility window, but old config/session files remain readable when deterministically mappable. Unknown selections (config, CLI, Web, RPC) fail explicitly. New assistant messages and session files store `ModelRef` (bumped `SESSION_VERSION`); historical cost resolves each message's exact provider card. Vertex's deployment `modelMap` is a separate concern and is unchanged.

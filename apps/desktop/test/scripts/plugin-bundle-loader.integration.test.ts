 ...
 *
 *   Build-time: apps/desktop/scripts/lib/defaults.ts → createPluginBundlePlan → bun build
 ...
 */
test("plugin bundled by prepareDefaultsResources satisfies runtime plugin-loader contract", async () => {
  ...
  const pluginDir = join(packageDir, "plugins", "test-plugin");
...
`,
  ...
  try {
    ...
    const outDir = join(defaultsResourcesDir, "plugins", "test-plugin");

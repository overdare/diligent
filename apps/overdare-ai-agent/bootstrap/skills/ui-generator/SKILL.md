...
5. If you touched a script, validate with `studiorpc_lua_validate`. Save, then report exactly what changed.
...
8. Add behavior only if requested — `LocalScript` + `Activated`, referencing runtime UI from `PlayerGui`; validate with `studiorpc_lua_validate`. See `patterns/script-integration.md`.

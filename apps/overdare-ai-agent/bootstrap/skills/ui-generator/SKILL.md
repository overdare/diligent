...
8. Add behavior only if requested — `LocalScript` + `Activated`, referencing runtime UI from `PlayerGui`; the write result carries Studio's validation report. See `patterns/script-integration.md`.

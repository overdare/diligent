...
6. **Generated image:** use `generate_image` for one bespoke icon, panel, or illustration. Its default `auto` selection uses the configured Gemini API key with Gemini's native image model, or falls back to local Codex managed ChatGPT OAuth when no Gemini key is configured. It saves the result locally and returns its provider, absolute `file` path, and preview. Do not substitute the public OpenAI Images API for the Codex OAuth path. Pass that exact `file` to `studiorpc_asset_manager_image_import`, then bind the returned `asset.assetid` exactly to the target `ImageLabel` or `ImageButton`.

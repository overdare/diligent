...
- Build static UI with `studiorpc_execute_luau` targeting `Editor`, not a gameplay script that recreates the entire screen at runtime. Reuse or create a `ScreenGui`, then create and parent containers before their children. Query unfamiliar classes or properties with `studiorpc_instance_schema_search` first.
- Change existing `Position`/`Size` through a focused Editor Luau command; use `studiorpc_instance_move` only to change parent hierarchy. Delete only an explicitly replaced target or your own temporary objects.

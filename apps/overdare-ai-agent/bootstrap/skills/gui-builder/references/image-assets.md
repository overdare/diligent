...
- Each call generates one image; there is no `n` tool input. For multiple assets or variants, issue one call per image in parallel in the same tool round. Use the same `referenceImages` in every call, with one composition per prompt. Wait for the shared guide or references to exist first; do not chain the previous output into the next call unless sequential edits were requested. Without references, omit them consistently. Preserve returned files and report any warning.

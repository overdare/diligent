...
- There is no `n` tool input. For multiple assets or variants, pass an array of image prompts in one `generate_image` call. The tool sends one `n: 1` HTTP request per prompt concurrently, using the same `referenceImages` for every request. Describe one composition per array entry. Wait for the shared guide or references to exist first; do not chain previous outputs into subsequent references unless sequential edits were requested. Without references, omit them. Preserve successful files and report warnings and failed prompt indexes; retry only failed images within their retry budget.

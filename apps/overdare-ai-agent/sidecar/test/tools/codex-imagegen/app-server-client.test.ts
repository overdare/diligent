...
test.each([
  "before",
  "after",
])("keeps image events %s turn/start acknowledgement and filters other threads", async (order) => {
  ...
  await child.stalled;
  const acknowledgement = { id: child.requests.at(-1)!.id, result: { turn: { id: "turn-1" } } };
  if (order === "after") child.stdout.write(`${JSON.stringify(acknowledgement)}\n`);
...
test("an image is not successful when the acknowledged turn ends without completion", async () => {
  const child = fakeProcess({ stallAt: "turn/start" });
  const generate = createGenerateCodexImage((options) => createCodexAppServer({ ...options, spawn: child.spawn }));
  const failure = generate({ cwd: process.cwd(), prompt: "A blue coin" }).catch((error: Error) => error.message);
  await child.stalled;
  child.stdout.write(`${JSON.stringify({ id: child.requests.at(-1)!.id, result: {} })}\n`);
  child.stdout.write(
    `${JSON.stringify({ method: "item/completed", params: { threadId: "thread-1", item: { type: "imageGeneration", savedPath: `${process.cwd()}/image.png` } } })}\n`,
  );
  child.stdout.end();
  expect(await failure).toContain("output closed");
  expect(child.exited()).toBe(true);
});
...
test("a late turn/start error rejects an otherwise completed image", async () => {
  const child = fakeProcess({ stallAt: "turn/start" });
  const generate = createGenerateCodexImage((options) => createCodexAppServer({ ...options, spawn: child.spawn }));
  const failure = generate({ cwd: process.cwd(), prompt: "A blue coin" }).catch((error: Error) => error.message);
  await child.stalled;
  for (const message of [
    {
      method: "item/completed",
      params: { threadId: "thread-1", item: { type: "imageGeneration", savedPath: `${process.cwd()}/image.png` } },
    },
    { method: "turn/completed", params: { threadId: "thread-1", turn: { status: "completed" } } },
    { id: child.requests.at(-1)!.id, error: { code: -1, message: "turn rejected" } },
  ])
    child.stdout.write(`${JSON.stringify(message)}\n`);
  expect(await failure).toContain("turn rejected");
  expect(child.exited()).toBe(true);
});

test("invalid completion data fails immediately instead of timing out", async () => {
  const child = fakeProcess({ stallAt: "turn/start" });
  const generate = createGenerateCodexImage((options) => createCodexAppServer({ ...options, spawn: child.spawn }), {
    timeoutMs: 100,
  });
  ...
  await child.stalled;
  child.stdout.write(
    `${JSON.stringify({ method: "turn/completed", params: { threadId: "thread-1", turn: { status: 123 } } })}\n`,
  );
  expect(await failure).toContain("invalid turn/completed");
  expect(child.exited()).toBe(true);
});

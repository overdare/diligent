...
test.each(["codex", "gemini"])("MCP cancellation reaches the %s image provider", async (provider) => {
  ...
  try {
    const result = client.callTool(
      { name: "generate_image", arguments: { provider, prompt: "A blue coin" } },
      undefined,
...
test("HTTP request cancellation reaches the image provider", async () => {
  ...
  const request = new Request(url, {
    ...
    body: JSON.stringify({ tool: "generate_image", args: { provider: "gemini", prompt: "A blue coin" } }),
    signal: controller.signal,
  });

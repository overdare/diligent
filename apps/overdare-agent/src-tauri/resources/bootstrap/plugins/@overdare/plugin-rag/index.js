...
var NEVER = INVALID;
// apps/overdare-agent/plugins/plugin-rag/src/render.ts
...
function summarizeSearchOutput(source, count) {
  if (count === 0)
...
function buildCodePreviewBlock(result) {
...
function buildDocsPreviewBlock(result) {
...
function buildAssetPreviewBlock(result) {
  const blocks = [
    {
      type: "key_value",
      title: "Top asset",
...
function buildSearchRender(args, results) {
  if (args.source === "assets") {
    const assetResults = results;
    const rows2 = assetResults.slice(0, 10).map((entry) => [
      clip(entry.title, 28),
    ...
    return {
      ...
      blocks: [
        ...
        ...assetResults.length === 0 ? [{ type: "summary", text: "No results found.", tone: "warning" }] : [],
        ...rows2.length > 0 ? [
          {
            type: "table",
            title: "Assets",
  ...
  const previewBlock = args.source === "code" ? buildCodePreviewBlock(results[0] ?? { text: "" }) : args.source === "docs" ? buildDocsPreviewBlock(results[0] ?? { text: "" }) : undefined;
  return {
    inputSummary: clip(`${args.source}: ${args.query}`, 100),
    ...
    blocks: [
      ...
      ...results.length === 0 ? [{ type: "summary", text: "No results found.", tone: "warning" }] : [],
      ...rows.length > 0 ? [{ type: "table", title: "Matches", columns: ["Snippet", "Origin"], rows }] : [],
      ...previewBlock ? [previewBlock] : []
    ]
  };
}
function buildOriginFileRender(action, requestedUrls, files) {
  const loaded = files.filter((entry) => typeof entry.content === "string");
  const rows = files.slice(0, 10).map((entry) => [
...
// apps/overdare-agent/plugins/plugin-rag/src/overdaresearch.ts
var BASE_URL = "https://aiguide.overdare.com";
var TIMEOUT_MS = 1e4;
function isAssetResult(result) {
...
var name = "overdaresearch";
var description = `Searches OVERDARE documentation, code examples, and assets using RAG.
...
var parameters = exports_external.object({
  query: exports_external.string().describe("Search query for OVERDARE (English only)"),
  source: exports_external.enum(["docs", "code", "assets"]).describe("docs = API references and guides. code = working Lua implementation examples and patterns. assets = asset catalog search with asset metadata fields."),
  topK: exports_external.number().int().min(1).max(10).describe("Number of results to return")
});
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (errJson?.error)
          errMsg = errJson.error.substring(0, 200);
      } catch {}
    ...
    const results = (data?.results ?? []).filter((result) => {
      if ((result.text ?? "").length > 0)
        return true;
      if ("script" in result && (result.script ?? "").length > 0)
        return true;
      return false;
    });
    if (args.source === "assets") {
      ...
      return {
        output: assetResults.length ? JSON.stringify({ results: assetResults, totalCount: data?.totalCount ?? assetResults.length }, null, 2) : "No results found.",
    ...
    const ragResults = results.filter((result) => !isAssetResult(result));
    return {
...
// apps/overdare-agent/plugins/plugin-rag/src/overdaresearch_deep.ts
var BASE_URL2 = "https://aiguide.overdare.com";
var TIMEOUT_MS2 = 5000;
var LUA_BUCKET = "https://storage.googleapis.com/lua-script-bucket/";
var DOCS_BUCKET = "https://storage.googleapis.com/ovdr-docs-bucket/";
var name2 = "overdaresearch_deep";
var description2 = `Fetches original file contents and related metadata from OVERDARE after an initial overdaresearch.
...
var parameters2 = exports_external.object({
  action: exports_external.enum(["origin-file"]).describe("origin-file = fetch full content of original files by GCS URL."),
  urls: exports_external.array(exports_external.string()).min(1).max(10).describe("For origin-file: 1-10 GCS file URLs (lua-script-bucket or ovdr-docs-bucket).")
});
...
function buildResult(action, requestedUrls, files, totalCount) {
  const loaded = files.filter((f) => f.content !== null);
  return {
    output: files.length ? JSON.stringify(files, null, 2) : `No ${action === "origin-file" ? "files returned" : "related metadata found"}.`,
    render: buildOriginFileRender(action, requestedUrls, files),
    metadata: { action, totalCount, loadedCount: loaded.length, files }
  };
}
async function originFile(urls, signal) {
...
Allowed: lua-script-bucket, ovdr-docs-bucket`);
  ...
  const data = await fetcher(`${BASE_URL2}/api/chat/rag/origin-file?${params}`, signal);
...
// apps/overdare-agent/plugins/plugin-rag/src/index.ts
var manifest = {
  name: "@overdare/plugin-rag",
  apiVersion: "1.0",
  version: "0.1.0"
};
async function createTools(_ctx) {
  return [
    {
      name,
      description,

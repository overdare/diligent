...
// apps/overdare-agent/plugins/plugin-validator/src/validatelua.ts
...
var NEVER = INVALID;
// apps/overdare-agent/plugins/plugin-validator/src/config.ts
...
// apps/overdare-agent/plugins/plugin-validator/src/render.ts
function clip(value, max = 80) {
  return value.length > max ? `${value.slice(0, max - 1).trimEnd()}\u2026` : value;
}
function buildValidateLuaRender(filePath, output) {
...
`).map((line) => line.trim()).filter(Boolean);
  ...
  return {
    ...
    blocks: [
      ...
      ...statusItems.length > 0 ? [{ type: "list", title: "Files", items: statusItems }] : [],
...
async function findLuaFiles(dirPath) {
  const results = [];
...
async function ensureSetupFiles(dir) {
  const luaurcPath = join2(dir, ".luaurc");
  if (!existsSync(luaurcPath)) {
    await writeFile(luaurcPath, JSON.stringify({ languageMode: "strict" }, null, 2), "utf-8");
  }
  const sourcemapPath = join2(dir, "sourcemap.temp.json");
  if (!existsSync(sourcemapPath)) {
    await writeFile(sourcemapPath, JSON.stringify({ name: "THIS_IS_TEMPORAL_SOURCEMAP", className: "DataModel" }, null, 2), "utf-8");
  }
  return sourcemapPath;
}
async function removeBOM(filePath) {
...
function filterOutput(raw) {
  return raw.split(`
`).filter((line) => !line.includes("[INFO]") && !line.includes("'Instance'")).join(`
`).trim();
}
...
function resolveLuauLspPath() {
  if (process.env.LUAU_LSP_PATH)
    return process.env.LUAU_LSP_PATH;
  const cfg = loadOverdareConfig();
  if (cfg.luauLspPath)
    return cfg.luauLspPath;
  const bin = process.platform === "win32" ? "luau-lsp.exe" : "luau-lsp";
  const bundled = join2(getPluginDir(), bin);
  return existsSync(bundled) ? bundled : "luau-lsp";
}
function resolveTypesPath() {
  if (process.env.OVERDARE_TYPES_PATH)
    return process.env.OVERDARE_TYPES_PATH;
  const cfg = loadOverdareConfig();
  if (cfg.typesPath)
    return cfg.typesPath;
  const bundled = join2(getPluginDir(), "overdare-types.d.lua");
  return existsSync(bundled) ? bundled : undefined;
}
var name = "validatelua";
var description = `Validates Luau script code for type errors and lint warnings using luau-lsp analyze. Call this tool to ensure code quality.
...
var parameters = exports_external.object({
  filePath: exports_external.string().describe("Path to a Luau file or a folder containing Luau files to validate")
});
async function execute(args, ctx) {
  ...
  try {
    ...
    throw new Error(`luau-lsp not found at: ${luauLspPath}
...
` + `Download: https://github.com/JohnnyMorganz/luau-lsp/releases`);
  ...
  if (typesPath) {
    try {
      ...
      throw new Error(`Type definitions file not found: ${typesPath}
` + `Set the OVERDARE_TYPES_PATH environment variable to the correct path.`);
  ...
  const { stdout, stderr } = await execFileAsync(luauLspPath, analyzeArgs).catch((error) => ({
    stdout: error.stdout ?? "",
    stderr: error.stderr ?? ""
  }));
...
// apps/overdare-agent/plugins/plugin-validator/src/index.ts
var manifest = {

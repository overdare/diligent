// @summary First-run setup: deploy bootstrap content (config, agents, skills, plugins) to ~/.diligent/
...
/// Check for updated bootstrap content at the legacy runtime path ~/.diligent/updates/runtime/defaults/.
...
/// Resolve the bundled bootstrap directory, trying multiple candidate paths.
fn resolve_bootstrap_dir(app: &tauri::AppHandle, log: &mut String) -> Option<PathBuf> {
    // 1. resource_dir() / bootstrap
    match app.path().resource_dir() {
        Ok(resource_dir) => {
            let _ = writeln!(log, "[init] resource_dir = {}", resource_dir.display());
            let candidate = resource_dir.join("bootstrap");
    ...
    // 2. exe directory / bootstrap
    match std::env::current_exe() {
        Ok(exe) => {
            let _ = writeln!(log, "[init] exe = {}", exe.display());
            if let Some(exe_dir) = exe.parent() {
                let candidate = exe_dir.join("bootstrap");
    ...
    let _ = writeln!(log, "[init] bootstrap dir not found — skipping deploy");
    None
}
...
/// Deploy bootstrap content with policy:
/// - MissingOnly (default): copy only when target does not exist
/// - FullSync (after update applied): replace with bundled/updated bootstrap content
...
pub fn run(app: &tauri::AppHandle, update_applied: bool) {
    ...
    // Prefer updated bootstrap from auto-update, fall back to bundled bootstrap
    let bootstrap_dir = resolve_updated_bootstrap_dir(&mut log)
        .or_else(|| resolve_bootstrap_dir(app, &mut log));

    if let Some(ref bootstrap) = bootstrap_dir {
        match fs::read_dir(bootstrap) {
            Err(e) => {
                let _ = writeln!(log, "[init] Cannot read bootstrap dir: {e}");
            }
            Ok(entries) => {

...
mod tests {
    ...
    fn parses_equals_form_cwd_argument() {
        let result = parse_startup_cwd_from_args(["overdare-agent-desktop", "--cwd=/tmp/project"]);
        assert_eq!(result.as_deref(), Some("/tmp/project"));
    }

    #[test]
    fn parses_split_form_cwd_argument() {
        let result = parse_startup_cwd_from_args(["overdare-agent-desktop", "--cwd", "./project"]);
        assert_eq!(result.as_deref(), Some("./project"));
    }

    #[test]
    fn ignores_missing_cwd_value() {
        let result = parse_startup_cwd_from_args(["overdare-agent-desktop", "--cwd"]);
    ...
    fn parses_equals_form_userid_argument() {
        let result = parse_startup_userid_from_args(["overdare-agent-desktop", "--userid=test-user"]);

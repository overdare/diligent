...
if (!VERSION) {
  console.error("Usage: bun run apps/overdare-agent/scripts/build-exe-only.ts <semver>");
  process.exit(1);
}
...
function collectDesktopExe(projectArtifactName: string): string {
  ...
  const candidatePaths = [
    join(rootReleaseDir, "overdare-agent-desktop.exe"),
    join(tripleReleaseDir, "overdare-agent-desktop.exe"),
  ];
